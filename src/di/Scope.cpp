...
namespace
{
	...
	) 
	{
		if (resolutionStack.size() <= 1) {
			return std::format(
				"Mach DI error: service is not registered: {}",
				missingType.name()
			);
		}

		const std::type_index parentType =
			resolutionStack[resolutionStack.size() - 2];
	...
	{
		std::string message =
			"Mach DI error: circular dependency detected: ";

		for (
			auto& it = cycleStart; it != resolutionStack.end(); ++it) {
			message += it->name();
			message += " -> ";
		}
...
namespace mach::detail::di
{
	std::shared_ptr<void> Scope::resolve(std::type_index type) {
		const auto cycleStart = std::find(
			m_resolutionStack.begin(),
			m_resolutionStack.end(),
			type
		);

		if (cycleStart != m_resolutionStack.end()) {
			throw std::logic_error(
				buildCircularDependencyMessage(
					cycleStart,
					type,
					m_resolutionStack
				)
		...
		if (descriptor == nullptr) {
			throw std::logic_error(
				buildMissingRegistrationMessage(
					type,
					m_resolutionStack
				)
			);
