...
namespace mach::detail::server
{
    ...
    net::awaitable<bool> BeastSession::do_read() {
        ...
        if (ec == http::error::end_of_stream || ec == net::error::eof ||
            ec == net::error::connection_reset || ec == net::error::connection_aborted ||
            ec == net::error::operation_aborted || ec == beast::error::timeout) {
            co_return false;
        }
        ...
        if (req.target().size() > maxTargetSize) {
            co_return co_await send_response(
                makeReadErrorResponse(mach::http::StatusCode::UriTooLong));
        }
