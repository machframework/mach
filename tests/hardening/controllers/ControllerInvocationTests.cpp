...
namespace
{
    ...
    public:
        ...
        explicit InvocationController(InvocationService& service)
            : m_service(service)
        {}

        static void configure(
            mach::ControllerBuilder<InvocationController>& builder
        )
        {
            builder.mapGet(
                "/plain",
                &InvocationController::plain
            );

            builder.mapGet(
                "/dependency",
                &InvocationController::dependency
            );

            builder.mapGet(
                "/context",
                &InvocationController::contextParameter
            );

            builder.mapGet(
                "/base-context",
                &InvocationController::baseContext
            );

            builder.mapGet(
                "/users/{id:int}",
                &InvocationController::routeParameter
            );

            builder.mapPost(
                "/body",
                &InvocationController::body
            );

            builder.mapGet(
                "/created",
                &InvocationController::createdResult
            );
        }
        ...
        mach::Reply<CreateUserRequest> body(
...
int main()
{
    ...
    std::cout
        << "Controller invocation test server started.\n\n"
