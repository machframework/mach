...
namespace
{
    ...
    class Consumer {
    public:
        explicit Consumer(ImmovableService& service) noexcept : m_service(service) {}
    ...
    void testTransientImmovableServiceCanBeResolved() {
        constexpr std::string_view testName = "Transient immovable service can be resolved";
        ...
        try {
            auto& first = scope.resolve<ImmovableService>();

            auto& second = scope.resolve<ImmovableService>();

            if (first.value() != 42 || second.value() != 42) {
                testing::fail(testName, "Resolved service returned an unexpected value");

                return;
            }

            if (&first == &second) {
                testing::fail(testName, "Transient resolutions returned the same instance");
    ...
    void testScopedImmovableServiceCanBeResolved() {
        constexpr std::string_view testName = "Scoped immovable service can be resolved";
        ...
        try {
            ...
            if (&first != &second) {
                testing::fail(testName, "Scoped resolutions returned different instances");
    ...
    void testSingletonImmovableServiceCanBeResolved() {
        constexpr std::string_view testName = "Singleton immovable service can be resolved";
        ...
        try {
            auto& first = firstScope.resolve<ImmovableService>();

            auto& second = secondScope.resolve<ImmovableService>();
            ...
            if (&first != &second) {
                testing::fail(testName, "Singleton resolutions returned different instances");
    ...
    void testImmovableServiceCanBeInjectedByReference() {
        constexpr std::string_view testName = "Immovable service can be injected by reference";

        di::Container container;

        container.addService<ImmovableService>(di::ServiceLifetime::Scoped);

        container.addService<Consumer, ImmovableService>(di::ServiceLifetime::Transient);
        ...
        try {
            ...
            auto& consumer = scope.resolve<Consumer>();

            if (&consumer.service() != &service) {
                testing::fail(testName, "Consumer received a different service instance");
