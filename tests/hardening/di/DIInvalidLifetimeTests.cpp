...
namespace
{
    ...
    public:
        explicit DirectScopedConsumer(ScopedDependency& dependency) noexcept
    ...
    public:
        explicit TransientScopedConsumer(ScopedDependency& dependency) noexcept
            : m_dependency(dependency) {}
    ...
    public:
        explicit IndirectScopedConsumer(TransientScopedConsumer& dependency) noexcept
            : m_dependency(dependency) {}
    ...
    public:
        explicit MiddleTransientConsumer(TransientScopedConsumer& dependency) noexcept
            : m_dependency(dependency) {}
    ...
    public:
        explicit DeepScopedConsumer(MiddleTransientConsumer& dependency) noexcept
            : m_dependency(dependency) {}
    ...
    public:
        explicit SingletonConsumer(SingletonDependency& dependency) noexcept
            : m_dependency(dependency) {}
    ...
    public:
        explicit TransientConsumer(TransientDependency& dependency) noexcept
            : m_dependency(dependency) {}
    ...
    public:
        explicit ScopedConsumer(ScopedDependency& dependency) noexcept : m_dependency(dependency) {}
    ...
    void testSingletonCannotDirectlyDependOnScoped() {
        constexpr std::string_view testName = "Singleton cannot directly depend on scoped service";

        di::Container container;

        container.addService<ScopedDependency>(di::ServiceLifetime::Scoped);

        container.addService<DirectScopedConsumer, ScopedDependency>(
            di::ServiceLifetime::Singleton);

        try {
            container.finalizeRegistrations();

            testing::fail(testName, "Finalization accepted a singleton with a scoped dependency");
    ...
    void testSingletonCannotIndirectlyDependOnScoped() {
        ...
        container.addService<IndirectScopedConsumer, TransientScopedConsumer>(
            di::ServiceLifetime::Singleton);

        try {
            container.finalizeRegistrations();

            testing::fail(testName, "Finalization accepted an indirect scoped dependency");
        } catch (const std::logic_error&) {
            testing::success(testName);
        } catch (const std::exception& exception) {
            testing::fail(testName, exception.what());
    ...
    void testDeepScopedDependencyIsRejected() {
        ...
        container.addService<DeepScopedConsumer, MiddleTransientConsumer>(
            di::ServiceLifetime::Singleton);

        try {
            container.finalizeRegistrations();

            testing::fail(testName, "Finalization accepted a deeply nested scoped dependency");
        } catch (const std::logic_error&) {
            testing::success(testName);
        } catch (const std::exception& exception) {
            testing::fail(testName, exception.what());
    ...
    void testSingletonCanDependOnSingleton() {
        constexpr std::string_view testName = "Singleton can depend on singleton";
    ...
    void testSingletonCanDependOnTransient() {
        ...
        container.addService<TransientConsumer, TransientDependency>(
            di::ServiceLifetime::Singleton);
    ...
    void testScopedCanDependOnScoped() {
        constexpr std::string_view testName = "Scoped service can depend on scoped service";

        di::Container container;

        container.addService<ScopedDependency>(di::ServiceLifetime::Scoped);

        container.addService<ScopedConsumer, ScopedDependency>(di::ServiceLifetime::Scoped);
    ...
    void testTransientCanDependOnScoped() {
        constexpr std::string_view testName = "Transient service can depend on scoped service";
