...
namespace
{
    ...
    class FirstConsumer {
    public:
        explicit FirstConsumer(Dependency& dependency) noexcept : m_dependency(dependency) {}
    ...
    public:
        explicit SecondConsumer(Dependency& dependency) noexcept : m_dependency(dependency) {}
    ...
    void testScopedDependencySharedWithinScope() {
        ...
        try {
            auto& firstConsumer = scope.resolve<FirstConsumer>();

            auto& secondConsumer = scope.resolve<SecondConsumer>();

            if (&firstConsumer.dependency() != &secondConsumer.dependency()) {
                testing::fail(testName, "Consumers received different scoped dependency instances");
    ...
    void testScopedDependencyDifferentAcrossScopes() {
        constexpr std::string_view testName = "Scoped dependency differs between scopes";
        ...
        try {
            auto& firstConsumer = firstScope.resolve<FirstConsumer>();

            auto& secondConsumer = secondScope.resolve<FirstConsumer>();
    ...
    void testSingletonDependencySharedAcrossScopes() {
        constexpr std::string_view testName = "Singleton dependency shared across scopes";
    ...
    void testTransientDependencyDifferentWithinScope() {
        ...
        try {
            ...
            auto& secondConsumer = scope.resolve<FirstConsumer>();
    ...
    void testTransientDependencyDifferentAcrossScopes() {
        constexpr std::string_view testName = "Transient dependency differs across scopes";
    ...
    void testScopedConstructionFailureDoesNotPoisonCache() {
        constexpr std::string_view testName = "Scoped construction failure does not poison cache";
        ...
        try {
            ...
            testing::fail(testName, "First scoped resolution unexpectedly succeeded");
        ...
        try {
            auto& firstSuccessfulResolution = scope.resolve<ThrowingService>();

            auto& secondSuccessfulResolution = scope.resolve<ThrowingService>();

            if (&firstSuccessfulResolution != &secondSuccessfulResolution) {
                testing::fail(testName, "Successful scoped instance was not cached");
    ...
    void testSingletonConstructionFailureDoesNotPoisonCache() {
        ...
        try {
            [[maybe_unused]]
            auto& service = firstScope.resolve<ThrowingService>();

            testing::fail(testName, "First singleton resolution unexpectedly succeeded");
        ...
        try {
            auto& firstSuccessfulResolution = firstScope.resolve<ThrowingService>();

            auto& secondSuccessfulResolution = secondScope.resolve<ThrowingService>();
    ...
    void testScopedInstanceDestroyedWithScope() {
        constexpr std::string_view testName = "Scoped instance destroyed with scope";

        TrackedService::reset();

        di::Container container;

        container.addService<TrackedService>(di::ServiceLifetime::Scoped);
...

        {
            ...
            if (TrackedService::aliveCount() != 1) {
                testing::fail(testName, "Scoped instance was not alive after resolution");
        ...
        if (TrackedService::aliveCount() != 0) {
            testing::fail(testName, "Scoped instance survived scope destruction");
    ...
    void testUnusedScopeConstructsNothing() {
        constexpr std::string_view testName = "Unused scope constructs no services";
        ...
        if (TrackedService::aliveCount() != 0) {
            testing::fail(testName, "Creating an unused scope constructed a service");
