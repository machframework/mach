...
int main() {
    ...
    try {
        ...
        std::cout << testing::GREEN
                  << "[SUCCESS] Duplicate route insertion threw std::logic_error: " << e.what()
                  << testing::RESET << std::endl;
    } catch (...) {
        std::cerr << testing::RED << "[FAIL] Wrong exception type thrown" << testing::RESET
                  << std::endl;

        return 1;
    }

    std::cout << testing::GREEN << "[SUCCESS] Duplicate route tests passed!" << testing::RESET
              << std::endl;
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Get, {"users"});

        if (match.status != mach::detail::routing::RoutingStatus::Found) {
            fail(testName, mach::detail::routing::RoutingStatus::Found, match.status);
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Get, {"users", "names"});
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Post, {"users"});
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Get, {"api", "v1", "users"});
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Delete, {"api", "v1", "users"});
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Get, {"health"});
    ...
    std::cout << testing::GREEN << "[SUCCESS] Found route tests passed!" << testing::RESET
              << std::endl;
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Get, {"orders"});

        if (match.status != mach::detail::routing::RoutingStatus::NotFound) {
            fail(testName, mach::detail::routing::RoutingStatus::NotFound, match.status);
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Get, {"users", "emails"});
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Get, {"api", "v2", "users"});
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Get, {"users", "names", "asc"});
    ...
    std::cout << testing::GREEN << "[SUCCESS] Not Found tests passed!" << testing::RESET
              << std::endl;
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Put, {"users"});

        if (match.status != mach::detail::routing::RoutingStatus::MethodNotAllowed) {
            fail(testName, mach::detail::routing::RoutingStatus::MethodNotAllowed, match.status);
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Delete, {"users", "names"});
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Post, {"api", "v1", "posts"});
...

    {
        ...
        auto match = t.matchRoute(mach::http::Method::Head, {"health"});
    ...
    std::cout << testing::GREEN << "[SUCCESS] Method Not Allowed tests passed!" << testing::RESET
              << std::endl;
...
void fail(
    ...
    mach::detail::routing::RoutingStatus expected,
    mach::detail::routing::RoutingStatus actual) {
    std::cerr << testing::RED << "[FAIL] " << testName << ": status should've been '"
              << mach::detail::routing::toString(expected) << "' but is '"
              << mach::detail::routing::toString(actual) << "'" << testing::RESET << std::endl;
}