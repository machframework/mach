...
int main() {
    ...
    auto expectInvalidArgument = [](std::string_view testName, auto&& action) {
        try {
            ...
            std::cout << testing::RED << "[FAIL] " << testName
                      << " - expected std::invalid_argument but nothing was thrown"
                      << testing::RESET << std::endl;
            ...
            std::cout << testing::RED << "[FAIL] " << testName
                      << " - expected std::invalid_argument but got: " << ex.what()
                      << testing::RESET << std::endl;
        } catch (...) {
            std::cout << testing::RED << "[FAIL] " << testName
                      << " - expected std::invalid_argument but got unknown exception"
                      << testing::RESET << std::endl;
        }
    };
    ...
    auto expectNoThrow = [](std::string_view testName, auto&& action) {
        try {
            ...
            std::cout << testing::RED << "[FAIL] " << testName
                      << " - unexpected exception: " << ex.what() << testing::RESET << std::endl;
        } catch (...) {
            std::cout << testing::RED << "[FAIL] " << testName << " - unknown unexpected exception"
                      << testing::RESET << std::endl;
        }
    };
    ...
    std::cout << testing::GREEN << "[SUCCESS] Route parameter registration torture tests completed!"
              << testing::RESET << std::endl;
    ...
    std::cout << "\nManual Postman checks:\n"
              ...
              << "    GET http://127.0.0.1:3143/orders/-12  // depends whether your int constraint "
                 "allows negatives\n"
              << std::endl;
