...
namespace mach
{
    ...
    {
        template <typename T>
        concept HasPublicStaticRouteField = requires {
            { T::route } -> std::convertible_to<std::string_view>;
        };

        template <typename T>
        concept ControllerType = std::is_class_v<T> && std::derived_from<T, mach::ControllerBase>;

        template <typename T>
        concept MachController = ControllerType<T> && HasPublicStaticRouteField<T> &&
