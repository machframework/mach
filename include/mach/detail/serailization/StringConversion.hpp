...
namespace mach
{
    ...
    T fromString(std::string_view value)
    {
        ...
        if constexpr (std::same_as<ValueType, std::string>) {
            ...
            std::integral<ValueType> &&
            !std::same_as<ValueType, bool>
            ) {
            ValueType result{};
            ...
            if (ec != std::errc{} || ptr != value.data() + value.size()) {
                throw std::invalid_argument(
                    "Mach error: failed to convert string to integer"
                );
            }
            ...
            throw std::invalid_argument(
                "Mach error: failed to convert string to bool"
            );
        }
        else {
            static_assert(
                std::same_as<ValueType, void>,
                "Mach error: unsupported target type"
                );
        }
    }
}
